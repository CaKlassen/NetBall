    ...
    public class QuadTree<T> where T : class, IQuadObject
    {
        ...
        public void Insert(T quadObject)
        {
            lock (syncLock)
            {
                ...
                Rectangle bounds = quadObject.rectangle;
                if (root == null)
                {
                    ...
                    rootSize = new Vector2(minLeafSize.X * (float)multiplier, minLeafSize.Y * (float)multiplier);
                    var center = new Point(bounds.X + bounds.Width / 2, bounds.Y + bounds.Height / 2);
                    var rootOrigin = new Point(center.X - (int)rootSize.X / 2, center.Y - (int)rootSize.Y / 2);
                    root = new QuadNode(new Rectangle(rootOrigin, rootSize.ToPoint()));
                }
        ...
        private void Query(Rectangle bounds, QuadNode node, List<T> results)
        {
            lock (syncLock)
            {
                ...
                if (bounds.Intersects(node.Bounds))
                {
                    ...
                    {
                        if (bounds.Intersects(quadObject.rectangle))
        ...
        private void InsertNodeObject(QuadNode node, T quadObject)
        {
            lock (syncLock)
            {
                if (!node.Bounds.Contains(quadObject.rectangle))
                ...
                if (!node.HasChildNodes() && node.Objects.Count + 1 > maxObjectsPerLeaf)
                {
                    ...
                    foreach (T childObject in childObjects)
                    {
                        foreach (QuadNode childNode in node.Nodes)
                        {
                            ...
                            if (childNode.Bounds.Contains(childObject.rectangle))
                            {
                ...
                foreach (QuadNode childNode in node.Nodes)
                {
                    if (childNode != null)
                    {
                        if (childNode.Bounds.Contains(quadObject.rectangle))
                        {
        ...
        void quadObject_BoundsChanged(object sender, EventArgs e)
        {
            lock (syncLock)
            {
                ...
                {
                    QuadNode node = objectToNodeLookup[quadObject];
                    if (!node.Bounds.Contains(quadObject.rectangle) || node.HasChildNodes())
                    {
                        RemoveQuadObjectFromNode(quadObject);
                        Insert(quadObject);
                        if (node.Parent != null)
